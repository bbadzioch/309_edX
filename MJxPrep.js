/*
 * BB 2019.05.29 slightly modified, so that ** renders as an exponent
 *
 *
...
if (window.MJxPrep) {
  ...
  function preProcessEqn(eqn) {
    ...
    // Python exponents a**b -> a^b
    //eqn = eqn.replace(/\*\*/g, "^");
    // typeset None as text
    eqn = eqn.replace(/None/g, "sf(No)sf(n)sf(e)");
    // typeset Any as text
    eqn = eqn.replace(/Any/g, "sf(Any)");
